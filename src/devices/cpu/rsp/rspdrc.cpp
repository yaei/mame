...
void rsp_device::generate_checksum_block(drcuml_block &block, compiler_state &compiler, const opcode_desc *seqhead, const opcode_desc *seqlast)
{
	...
	if (!(m_drcoptions & RSPDRC_STRICT_VERIFY) || seqhead->next() == nullptr)
	{
		if (!(seqhead->flags & OPFLAG_VIRTUAL_NOOP))
		{
			...
			void *base = space(AS_PROGRAM).get_read_ptr(seqhead->physpc | 0x1000);
			UML_LOAD(block, I0, base, 0, SIZE_DWORD, SCALE_x4);                         // load    i0,base,0,dword

			if (seqhead->delay.first() != nullptr && seqhead->physpc != seqhead->delay.first()->physpc)
			{
				base = space(AS_PROGRAM).get_read_ptr((seqhead->delay.first()->physpc & 0x00000fff) | 0x1000);
				assert(base != nullptr);
	...
	else
	{
		...
		for (curdesc = seqhead->next(); curdesc != seqlast->next(); curdesc = curdesc->next())
			...
			{
				base = space(AS_PROGRAM).get_read_ptr(curdesc->physpc | 0x1000);
				assert(base != nullptr);
				UML_LOAD(block, I1, base, 0, SIZE_DWORD, SCALE_x4);                     // load    i1,base,dword
				UML_ADD(block, I0, I0, I1);                         // add     i0,i0,i1
				...
				{
					base = space(AS_PROGRAM).get_read_ptr((curdesc->delay.first()->physpc & 0x00000fff) | 0x1000);
					assert(base != nullptr);
					UML_LOAD(block, I1, base, 0, SIZE_DWORD, SCALE_x4);                 // load    i1,base,dword
					UML_ADD(block, I0, I0, I1);                     // add     i0,i0,i1
