...
void mbee_state::init_mbee128()
{
	...
	{
		m_bankr[b_bank]->configure_entries(0, 64, &RAM[0x0000], 0x1000); // RAM banks
		m_bankr[b_bank]->configure_entries(64, 4, &ROM[0x0000], 0x1000); // rom

		m_bankw[b_bank]->configure_entries(0, 64, &RAM[0x0000], 0x1000); // RAM banks
		m_bankw[b_bank]->configure_entries(64, 1, &ROM[0x4000], 0x1000); // dummy rom
	}
